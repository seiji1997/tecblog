...
function addAstroTypes(astroInstall, ts, host) {
    ...
    if (host.getProjectReferences && !host.getParsedCommandLine) {
        host.getParsedCommandLine = (fileName) => {
            ...
            const parsed = ts.parseJsonSourceFileConfigFileContent(configFile, ts.sys, basePath, undefined, fileName);
            parsed.fileNames = parsed.fileNames.filter((f) => !nonTsExtensions.some((ext) => f.endsWith(ext)));
            return parsed;
        };
    }
}
...
class AstroVirtualCode {
    constructor(fileName, snapshot) {
        ...
        const tsx = (0, astro2tsx_js_1.astro2tsx)(input, this.fileName);
        const frontmatter = (0, parseAstro_js_1.getFrontmatterStatus)(tsx.frontmatterStatus, tsx.frontmatterSource, input);
        const { htmlDocument, virtualCode: htmlVirtualCode } = (0, parseHTML_js_1.parseHTML)(this.snapshot, frontmatter.status === 'closed' ? frontmatter.position.end.offset : 0);
