...
function enhancedProvideCompletionItems(ts, completions, documentText) {
    const importedComponentSources = (0, utils_js_1.getAlreadyImportedComponentSources)(ts, documentText);
    const seenAstroComponents = new Set();
    completions.items = completions.items
        ...
        if (source?.endsWith('.astro')) {
            const key = `${(0, utils_js_1.stripAstroComponentSuffix)(String(completion.label))}\u0000${source}`;
        ...
        if (source) {
            ...
            if ((0, utils_js_1.isComponentImportSource)(source)) {
...
function enhancedResolveCompletionItem(resolvedCompletion, context) {
    ...
    if ((0, utils_js_1.isComponentImportSource)(resolvedCompletion.data.originalItem.source)) {
...
function rewriteAstroComponentCompletion(completion) {
    ...
    if (completion.textEdit && 'newText' in completion.textEdit) {
        completion.textEdit.newText = (0, utils_js_1.stripAstroComponentSuffix)(completion.textEdit.newText);
    }
    if (completion.additionalTextEdits) {
        completion.additionalTextEdits = completion.additionalTextEdits.map((edit) => ({
            ...edit,
            newText: (0, utils_js_1.rewriteComponentImportText)(edit.newText),
        }));
    }
}
