"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.isComponentImportSource = isComponentImportSource;
exports.stripAstroComponentSuffix = stripAstroComponentSuffix;
exports.rewriteComponentImportText = rewriteComponentImportText;
exports.getAlreadyImportedComponentSources = getAlreadyImportedComponentSources;
exports.mapEdit = mapEdit;
const utils_js_1 = require("../../core/utils.js");
const utils_js_2 = require("../utils.js");
const ASTRO_COMPONENT_SUFFIX = 'AstroComponent';
const COMPONENT_IMPORT_FROM_PATTERN = /\bfrom\s+['"][^'"]+\.(?:astro|svelte|vue)['"]/;
const ASTRO_DEFAULT_IMPORT_PATTERN = /^(\s*import(?:\s+type)?\s+)([A-Za-z_$][\w$]*)AstroComponent(?=\s*,|\s+from\b)/;
const ASTRO_DEFAULT_ALIAS_PATTERN = /(default\s+as\s+)([A-Za-z_$][\w$]*)AstroComponent(?=\s*\})/;
const ASTRO_NAMED_IMPORT_PATTERN = /^(\s*import\s+)\{\s*([A-Za-z_$][\w$]*)\s*\}(\s+from\s+['"]([^'"]+\.astro)['"])/;
function isComponentImportSource(source) {
    return (!!source && (source.endsWith('.astro') || source.endsWith('.svelte') || source.endsWith('.vue')));
}
function stripAstroComponentSuffix(name) {
    if (!name.endsWith(ASTRO_COMPONENT_SUFFIX)) {
        return name;
    }
    return name.slice(0, -ASTRO_COMPONENT_SUFFIX.length);
}
function rewriteComponentImportText(text) {
    return text
        .split('\n')
        .map((line) => {
        if (!COMPONENT_IMPORT_FROM_PATTERN.test(line)) {
            return line;
        }
        return rewriteComponentNamedImport(line)
            .replace(ASTRO_DEFAULT_IMPORT_PATTERN, '$1$2')
            .replace(ASTRO_DEFAULT_ALIAS_PATTERN, '$1$2');
    })
        .join('\n');
}
function rewriteComponentNamedImport(line) {
    const match = ASTRO_NAMED_IMPORT_PATTERN.exec(line);
    if (!match || match[2] !== (0, utils_js_1.classNameFromFilename)(match[4])) {
        return line;
    }
    return line.replace(ASTRO_NAMED_IMPORT_PATTERN, '$1$2$3');
}
function getAlreadyImportedComponentSources(ts, documentText) {
    const sources = new Set();
    const sourceFile = ts.createSourceFile('component-imports.tsx', getImportParseText(documentText), ts.ScriptTarget.Latest, false, ts.ScriptKind.TSX);
    for (const statement of sourceFile.statements) {
        if (!ts.isImportDeclaration(statement) || !ts.isStringLiteral(statement.moduleSpecifier)) {
            continue;
        }
        const source = statement.moduleSpecifier.text;
        const importClause = statement.importClause;
        if (!importClause || importClause.isTypeOnly || !isComponentImportSource(source)) {
            continue;
        }
        if (importClause.name) {
            sources.add(source);
            continue;
        }
        const namedBindings = importClause.namedBindings;
        if (!namedBindings || !ts.isNamedImports(namedBindings)) {
            continue;
        }
        if (namedBindings.elements.some((element) => element.propertyName?.text === 'default')) {
            sources.add(source);
        }
    }
    return sources;
}
function getImportParseText(documentText) {
    if (!documentText.startsWith('---')) {
        return documentText;
    }
    const lines = documentText.split('\n');
    if (lines[0]?.trim() !== '---') {
        return documentText;
    }
    const closingFrontmatterLine = lines.findIndex((line, index) => index > 0 && line.trim() === '---');
    return lines
        .slice(1, closingFrontmatterLine === -1 ? undefined : closingFrontmatterLine)
        .join('\n');
}
function mapEdit(edit, code, languageId) {
    // Don't attempt to move the edit to the frontmatter if the file isn't the root TSX file, it means it's a script tag
    if (languageId === 'typescriptreact') {
        if ((0, utils_js_2.editShouldBeInFrontmatter)(edit.range, code.astroMeta).itShould) {
            edit = (0, utils_js_2.ensureProperEditForFrontmatter)(edit, code.astroMeta, '\n');
        }
    }
    else {
        // If the edit is at the start of the file, add a newline before it; otherwise, we'll get `<script>text`
        if (edit.range.start.line === 0 && edit.range.start.character === 0) {
            edit.newText = '\n' + edit.newText;
        }
    }
    edit.newText = rewriteComponentImportText(edit.newText);
    return edit;
}
//# sourceMappingURL=utils.js.map